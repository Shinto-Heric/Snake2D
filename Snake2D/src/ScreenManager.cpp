...
// If you want these convenience methods, include your concrete screens here:
...
// Optional convenience functions (assume your screens can be default-constructed.
// If they need args, change to SwitchToScreen<YourScreen>(args...);)
void ScreenManager::SwitchToGameplay() {
    if (!m_game) return;
    // m_game->SetGameState(GameState::Playing); // if you keep state here
    SetScreen(std::make_unique<GameplayScreen>());
    std::cout << "Switched to GameplayScreen (queued)\n";
}

void ScreenManager::SwitchToMainMenu() {
    if (!m_game) return;
    // m_game->SetGameState(GameState::MainMenu);
    SetScreen(std::make_unique<MainMenuScreen>());
    std::cout << "Switched to MainMenuScreen (queued)\n";
}
