    ...
    : m_game(game), m_config(config), m_assets(assets)
{
    ...
    settingsBtn.SetCallback([]() { std::cout << "Settings pressed!" << std::endl; });
...
void MainMenuScreen::Update(sf::Time dt) {
    ...
    // Update buttons for hover animation
    for (auto& btn : m_buttons)
        btn.Update(dt);
}

void MainMenuScreen::Render(sf::RenderWindow& window) {
    window.clear(sf::Color(30, 30, 60)); // Background color
